...
class PreannouncedDeficit_OLG_Original:
    ...
    def solve_model(self, announce_t=None, implement_t=None):
        ...
        # End-of-period debt d_{t+1}, aligned with t-grid
...
class PreannouncedDeficit_OLG:
    ...
    def solve_model(self, announce_t=None, implement_t=None):
        ...
        # FIX: end-of-period debt d_{t+1}, aligned with t-grid
        self.sol.debt_end = np.empty_like(self.sol.d)
        self.sol.debt_end[:-1] = self.sol.d[1:]
        self.sol.debt_end[-1] = np.nan
    ...
    def plot_eps0_irfs(
        ...
        """
        Same visual style as Taud_taylor_OLG.plot_eps0_irfs(), but for the
        preannounced-deficit model.

        Panels:
          1) Output y_t
          2) End-of-period public debt d_{t+1}
          3) Inflation pi_t and real rate r_t
          4) Self-financing share nu(s) versus implementation date s
        """
        par = self.par
        ...
        try:
            ...
            # Solve IRFs for selected tau_d values at chosen implementation date
            selected_results = []
            ...
            with mpl.rc_context({
                ...
                ax.set_title(r"Inflation and real rate")
                ax.set_xlabel(r"$t$")
                ...
                # ---------- Panel 4: Self-financing share vs implementation horizon ----------
                ax = axes[1, 1]
